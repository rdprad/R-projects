...
# BM_df$elevation <- as.factor(BM_df$elevation)
# BM_df  <- saveRDS(BM_df, here::here("objects", "BM_df2.rds"))
test <- BM_df %>%
  filter(elevation>=930, elevation<=960)
# # 
# BM_df$elevation(10)
                 ...
                 sidebarLayout(
                   sidebarPanel(
                     ...
                     sliderTextInput(
                       ...
                       label = "",
                       choices = sort(unique(BM_df$elevation)),
                       selected = sort(unique(BM_df$elevation))[c(17, 20)]),
...
server <- function(input, output) {
  ...
  selected_blocks <- reactive({
    req(input$phaseselect, input$mySliderText, input$gradeitemselect)
    BM_df %>%
      filter(PhaseID %in% input$phaseselect,
             elevation >= as.numeric(input$mySliderText[1]),
             elevation <= as.numeric(input$mySliderText[2])) %>% 
