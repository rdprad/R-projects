...
con <- dbConnect(odbc::odbc(), .connection_string = "Driver={SQL Server};SERVER=MNOYTSQLD1")
...
library(dbplyr)
q1 <- tbl(con, "RawMMRSActivity") %>% 
  # filter(grepl('930', Fleet), grepl('Utilized', `Time Class`), Destination=="CRSH", `Time Code`=="Dump", Month==8) %>%
  filter(Fleet %like% '%930%', `Time Class` %like% '%Utilized%', Destination=="CRSH", `Time Code`=="Dump", Year ==2018, Month==8) %>%
  # arrange(Date, Shift) %>%
  group_by(Date, Shift, Location) %>%
  summarise(time = sum(`Time Usage`, na.rm = T))
q1

q1_df <- collect(q1)
q1_df <- xts::xts(q1_df, as.Date(q1_df$Date))
dygraph(q1_df)
