...
YTD_df <- Qcon %>% 
  ...
  summarise(Mass    = sum(Tonnes),
            ...
            # TPOH    = ifelse(SPI>0 & SPI<=60, 29320 * CI^0.19 * SPI^-0.36 * MB^-0.24,
                    ...
                    dashboardBody(
                      tags$head(
                        tags$style(HTML("
                         ...
                         #' .datatables {font-size: 1.5vw;}
                         #' @media screen and (min-width: 1024px) {.datatables {font-size: 12px;}}
                      tabItems(
                        # First tab content
                                ...
                                fluidRow(
                                  column(width = 12,
                                         fluidRow(
                                           column(width = 3, 
                                                  fluidRow(
                                                    column(width = 12, dateRangeInput("date", label = h4("Date range"), 
...
server <- function(input, output, session) {
  ...
  table_daily_column_1 <- reactive({
    Qcon %>%
      ...
      summarise(Crusher = accounting(round(sum(Tonnes),0),format = "d"),
                Cu      = round(weighted.mean(CU, Tonnes),3),
                Au      = round(weighted.mean(AU, Tonnes),3),
  ...
  table_daily_column_2 <- reactive({
    Qcon %>%
      ...
      mutate(OreType = "ALL",
             `Minor Source 2` = "ALL",
             Source2 = "ALL_POLY_SOURCES") %>%
      group_by(OreType, `Minor Source 2`, Source2) %>%
      summarise(Crusher = accounting(round(sum(Tonnes, na.rm = T),0),format = "d"),
                Cu      = round(weighted.mean(CU, Tonnes, na.rm = T),3),
                Au      = round(weighted.mean(AU, Tonnes, na.rm = T),3),
                Cu_rec  = round(weighted.mean(RECCU, Tonnes, na.rm = T)/Cu*100,1),
                Au_rec  = round(weighted.mean(RECAU, Tonnes, na.rm = T)/Au*100,1), 
                SPI     = accounting(weighted.mean(SPI, Tonnes, na.rm = T),format = "d"),
                MB      = accounting(weighted.mean(MB, Tonnes, na.rm = T),format = "d"),
                CI      = accounting(weighted.mean(CI, Tonnes, na.rm = T),format = "d"),
                TPOH    = accounting(1000/weighted.mean(HPKT, Tonnes, na.rm = T), format = "d"),
                AS      = accounting(weighted.mean(AS, Tonnes, na.rm = T),format = "d"),
                CONAS   = accounting(weighted.mean(CONAS, ConTon, na.rm = T),format = "d")) %>% 
      rename(`Minor Source` = `Minor Source 2`) %>% 
      rename(Source = Source2)
  })
  
  output$table1 <- DT::renderDataTable(server = FALSE,{
    return(as.datatable(formattable(table_daily_column_1(), list(
      Crusher  = color_tile("white", "lightgreen"),
      ...
      OreType  = formatter("span", style = x ~ ifelse(x == "Central Ore", 
                                                      formattable::style(color = 'rgba(25, 137, 250, 1)', font.weight = "bold"),
                                                      formattable::style(color = 'rgba(250, 137, 25, 1)', font.weight = "bold")))
    )),options = list(dom = 'Bft',buttons = c('copy', 'csv', 'excel'), scrollY = '300px', pageLength = 1000), extensions = 'Buttons', class = "display", rownames= FALSE))
  })
  
  output$table2 <- DT::renderDataTable({
    return(as.datatable(formattable(table_daily_column_2()
    ),options = list(dom = 't', bSort=FALSE, scrollY = '60px',pageLength = 1000), rownames= FALSE))
  })
  
  output$chart_pie_1 <- renderChart2({
    x = table_daily_pie_1()
    x$OreType <- factor(x$OreType, levels = c("SW Ore", "Central Ore", "NA"))
    p1 = hPlot(x = "OreType", y = "Mass", data = x, type = "pie")
    p1$colors('rgba(250, 137, 25, 0.8)','rgba(25, 137, 250, 0.8)','rgba(0, 0, 0, 0.8)')
  ...
  table_EU <- reactive({
    Query_PI_DF %>%
      ...
      mutate(Mill_OPhrs = ifelse(Milled_Ore_TPOH_Daily_Official>0, Milled_Ore_DMT_Daily_Official/Milled_Ore_TPOH_Daily_Official,0))
  })
  ...
  table_daily_comparison <- reactive({
    rbind(Qcon %>% 
            ...
            summarise(Crusher = accounting(sum(Tonnes),format = "d"),
                      Mill    = accounting(sum(table_EU()$Mill_OPhrs)*1000/weighted.mean(HPKT, Tonnes, na.rm = T),format = "d"),
                      Cu      = round(weighted.mean(CU, Tonnes, na.rm = T),3),
                      Au      = round(weighted.mean(AU, Tonnes, na.rm = T),3),
                      Cu_rec  = round(weighted.mean(RECCU, Tonnes, na.rm = T)/Cu*100,1),
                      Au_rec  = round(weighted.mean(RECAU, Tonnes, na.rm = T)/Au*100,1),
